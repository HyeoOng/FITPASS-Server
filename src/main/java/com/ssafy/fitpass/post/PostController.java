...
public class PostController {
    ...
    public Map<String, String> createPost(@RequestPart("post") Post post,
        ...
        if(!file.isEmpty())photo.setFile(file);
            ...
            response.put("msg", "사진 이상"); // UAL0003
            return response;
        }
...
//            throw new IllegalArgumentException("비난, 혐오, 욕설이 포함된 글은 작성할 수 없습니다.");
            response.put("msg", "fail with ai"); // UAL0005
            return response;
        }
            ...
            if(postService.createPost(post)){
                ...
                if(postService.createPostPhoto(photo)){
                    ...
                    try {
                        ...
                        response.put("msg", "파일 저장에 실패하였습니다. 잠시 후 다시 시도해주세요."); // SAL0001
                        return response; // 실패 시 바로 반환
                    }
                    response.put("msg", "글 등록에 성공하였습니다.");
                }else{
                    response.put("msg", "글은 등록했지만 이미지 등록에는 실패하였습니다."); // DAL0001
                }
            }else{ // 글 등록 실패
                response.put("msg", "글 등록에 실패하였습니다. <br> 잠시 후에 다시 시도해주세요."); // DAL0001
            }
    ...
    public Map<String, String> modifyPost(@RequestBody Post post){
        ...
        try{
            if(postService.modifyPost(post)){
                ...
                response.put("msg", "글 수정에 실패하였습니다. <br> 잠시 후 다시 시도해주세요."); // DAL0001
            }
            return response;
        }catch (RegDBException e){
            // DAL0001
